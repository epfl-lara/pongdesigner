...
object CustomDialogs {
   ...
   **/
  def launchChoiceDialogWithCustomchoice(context: Context, title: String, array: Int, funcOK: String => Unit, funcCanceled: () => Unit, defaultForCustom: String = "") {
    val alert = new AlertDialog.Builder(context)
  ...
  def launchChoiceDialogWithCustomchoice(context: Context, title: String, string_array: List[CharSequence], funcOK: String => Unit, funcCanceled: () => Unit) {
    ...
    alert.setTitle(title)
    alert.setItems(string_array.toArray, new DialogInterface.OnClickListener() {
       def onClick(dialog: DialogInterface, which: Int) {
         // The 'which' argument contains the index position
         // of the selected item
         if(which == 0) {
           ...
           funcOK(string_array(which).toString)
         }
       }
    });
    alert.show()
