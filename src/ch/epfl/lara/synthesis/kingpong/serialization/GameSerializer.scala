...
object GameSerializer {
  ...
  class SerializerContext(
    ...
    val map: MMap[(String, Int), GameObject] = MMap[(String, Int), GameObject](),
    val identifiers: MMap[(String, Int), Identifier] = MMap[(String, Int), Identifier](),
    val categories: MMap[(String, Int), CategoryObject] = MMap[(String, Int), CategoryObject]()) {
    def add(o: GameObject) = {
      if (o != null) {
        objects += o
        o.name.reset(game.interpreter)
        map += (o.name.get, o.id) -> o
      }
    }
    def add(c: CategoryObject) = {
      if (c != null && !(categories contains (c.name, c.id))) {
        categories += (c.name, c.id) -> c
      }
    }
  ...
  abstract class CustomConverter[T](TAG: String) extends Converter[T] {
    ...
    implicit class RichMap(s: String) {
      ...
      def <-=>(t: T => GameObject): Unit = map += ((s, e => gameRefToJson(t(e))))
      def <*=>(t: T => CategoryObject): Unit = map += ((s, e => categoryRefToJson(t(e))))
    ...
    implicit def internalJsonToGameObject(json: JSONObject): GameObject = implicitContext.map.getOrElse(jsonToNameId(json), {
      val expectedName = jsonToString(json)
      implicitContext.map.find({ case ((name, id), obj) => expectedName == name }) match {
        case Some(((name, id), obj)) => obj
        case None =>
          println("Error while importing " + jsonToString(json))
          null
      }
    })
    implicit def internalJsonToCategory(json: JSONObject): CategoryObject = implicitContext.categories.getOrElse(jsonToNameId(json), {
      val expectedName = jsonToString(json)
      implicitContext.categories.find({ case ((name, id), obj) => expectedName == name }) match {
        case Some(((name, id), obj)) => obj
        case None =>
          println("Error while importing " + jsonToString(json))
          null
  ...
  def jsonToNameId(json: JSONObject): (String, Int) = {
    if (json != null && json.has("name") && json.has("id")) {
      (json.getString("name"), json.getInt("id"))
    } else {
      null
    }
  }
  def nameIdtoJson(nameid: (String, Int)): JSONObject = {
    if (nameid == null) return null
    val json = new JSONObject()
    json.put("name", nameid._1)
    json.put("id", nameid._2)
    json
  }
  def gameRefToJson(g: GameObject): JSONObject = {
    if(g == null) return null
    nameIdtoJson((g.name.get, g.id))
  }
  def categoryRefToJson(c: CategoryObject): JSONObject = {
    if(c == null) return null
    nameIdtoJson(c.name, c.id)
  }
  ...
  def jsonToCategory(json: JSONObject, game: Game)(implicit ctx: SerializerContext): CategoryObject = {
    ...
    val id = { val id = json.optInt("id", -1); if (id == -1) UniqueCounter.nextGlobal else id }
    var created = false
    val jexpr = (str: String) => jsonToExprWithShortcut(json, str)
    val category = ctx.categories.getOrElse((name, id), {
      created = true; new CategoryObject(
        game,
        name,
        jexpr("angle"),
        jexpr("width"),
    ...
    if (created) {
      category.id = id
      ctx.categories += (name, id) -> category
      category
    } else {
      category
    }
  ...
  def gameObjectToJson(g: GameObject): JSONObject = {
    ...
    json.put("category", categoryRefToJson(g.category))
    json.put("id", g.id)
    for (p <- g.properties) {
      p match {
  ...
  def jsonToGameObject(json: JSONObject)(implicit game: Game, ctx: SerializerContext): Option[GameObject] = {
    ...
    val category: CategoryObject = ctx.categories.getOrElse(if (json has "category") jsonToNameId(json.optJSONObject("category")) else ("", 0), {
      val expectedName = json.optString("category", "%5%5%5");
      (ctx.categories.find { case ((name, id), cat) => name == expectedName }) match {
        case Some(((name, id), cat)) => cat
        case None =>
    ...
    var name = ""
    val obj: GameObject =
      if (cl == classOf[Rectangle].getName()) {
        rectangle(category)("", 0, 0, tpe = tpe)
      } else if (cl == classOf[Circle].getName()) {
        circle(category)("", 0, 0, tpe = tpe)
      } else if (cl == classOf[Character].getName()) {
        character(category)("", 0, 0, tpe = tpe)
      } else if (cl == classOf[IntBox].getName()) {
        intbox(category)("", 0, 0)
      } else if (cl == classOf[StringBox].getName()) {
        stringbox(category)("", 0, 0, "")
      } else if (cl == classOf[BooleanBox].getName()) {
        booleanbox(category)("", 0, 0, true)
      } else if (cl == classOf[Joystick].getName()) {
        joystick(category)("", 0, 0)
      } else if (cl == classOf[RandomGenerator].getName()) {
        randomGenerator(category)("", 0, 0)
      ...
      } else if (cl == classOf[SoundTTS].getName()) {
        soundTTS(category)("", 0, 0)
      } else if (cl == classOf[DrawingObject].getName()) {
        drawingObject(category)("", 0, 0)
        //} else if(cl == classOf[SoundRecorded].getName()) { soundrecorded(category)("", 0, 0)
      } else if (cl == classOf[SoundRecorder].getName()) {
        soundRecorder(category)("", 0, 0)
      } else if (cl == classOf[Gravity].getName()) {
        gravity(category)("", 0, 0)
      } else null
    ...
    if (obj != null) {
      obj.id = json.optInt("id", -1)
      if(obj.id == -1) obj.id = UniqueCounter.nextGlobal
  ...
  def gameToJson(game: Game, bitmaps: List[(Int, Bitmap)]): JSONObject = {
    ...
    /* Pre-processing on the game. Make sure all categories have unique names */
  ...
  def jsonToGame(json: JSONObject): Option[(Game, List[(Int, Bitmap)])] = {
    ...
    if (json.has("categories")) {
      for (
        j <- json.getJSONObjectSeq("categories");
    ...
    if (json.has("objects")) {
      for (
        j <- json.getJSONObjectSeq("objects");
        ...
        obj match {
          case array: Array2D =>
            array.cellsCategory = ctx.categories.getOrElse((obj.name.get, obj.id), array.cellsCategory)
          case _ =>
        }
      }
    }
